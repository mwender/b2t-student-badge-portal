...
(function($){
  ...
  var displayBadge = function( badge ){
    ...
    for( var i = 0; i < criteria.length; i++ ){
      // Our SalesForce API only provides a criteria.type
      // for Badges. Otherwise, we assume all criteria
      // are of type `Exam` for Certifications.
      if( 'badge' == badge.type ){
    ...
    $.getJSON( wpvars.criteriaurl, {name: badge.name}, function(data){
      $(badge.element + ' .alert').fadeOut(200);
