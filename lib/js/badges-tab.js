...
(function($){
  ...
  var modalTemplateHtml = $('#badge-assertion-modal-template').html();
  var modalSelector = '#b2t-assertion-modal';
  var modalOpenClass = 'is-open';
  var lastFocusedElement = null;

  function ensureModal() {
    if (!modalTemplateHtml) {
      return;
    }
    if (!$(modalSelector).length) {
      $('body').append(modalTemplateHtml);
    }
  }

  function setModalStatus(message, isError) {
    var $modal = $(modalSelector);
    var $status = $modal.find('.b2t-assertion-modal__status');
    if (!$status.length) {
      return;
    }
    $status.text(message || '');
  ...
  function openAssertionModal(assertionUrl) {
    ensureModal();
    var $modal = $(modalSelector);
    if (!$modal.length) {
      window.prompt('Copy this badge URL:', assertionUrl);
      return;
    }

    lastFocusedElement = document.activeElement;
    $modal.find('#b2t-assertion-url').val(assertionUrl);
    setModalStatus('');
    $modal.addClass(modalOpenClass).attr('aria-hidden', 'false');
    $('body').addClass('b2t-modal-open');
    $modal.find('[data-b2t-copy]').trigger('focus');
  }

  function closeAssertionModal() {
    var $modal = $(modalSelector);
    if (!$modal.length) {
      return;
    }
    $modal.removeClass(modalOpenClass).attr('aria-hidden', 'true');
    $('body').removeClass('b2t-modal-open');
    if (lastFocusedElement && typeof lastFocusedElement.focus === 'function') {
      lastFocusedElement.focus();
    }
  }

  function selectAssertionUrl() {
    var $input = $(modalSelector).find('#b2t-assertion-url');
    if (!$input.length) {
      return;
    }
    $input.trigger('focus');
  ...
  $(document).on('click', '[data-b2t-modal-close]', function() {
    closeAssertionModal();
  });

  $(document).on('click', '[data-b2t-select]', function() {
    selectAssertionUrl();
    setModalStatus('Badge URL selected. Use Cmd+C to copy.', false);
  });

  $(document).on('click', '[data-b2t-copy]', function() {
    var assertionUrl = $(modalSelector).find('#b2t-assertion-url').val();
    if (!assertionUrl) {
      return;
    }

    copyToClipboard(assertionUrl)
      .then(function() {
        setModalStatus('Copied to clipboard.', false);
      })
      .catch(function() {
        setModalStatus('Unable to copy automatically. Use Select and copy manually.', true);
      });
  });

  $(document).on('keydown', function(event) {
    if (event.key === 'Escape' && $(modalSelector).hasClass(modalOpenClass)) {
      closeAssertionModal();
    }
  });
  ...
  $('#badge-display, #certificate-display').on('click', '.add-to-backpack', function(e){
    ...
    $.ajax({
      ...
      success: function(assertion){
        ...
        openAssertionModal(assertion.id);
